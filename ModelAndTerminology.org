...
BISOS csPlayer is rooted in the model of interactve creation of command line and running it.
While the common structure of Command Line Interface (CLI) is assumed to have been understood,
it hardly is. The vocabulary used to refer to the anatomy of a command line is a mess and all
over the place.

In the context  PyCS and csPlayers, we have created our own reference model with a precise vocabular
and terminology.
...
- [[#general-form-of-pycs-parameters-options-and-arguments][General form of PyCS Parameters, Options and Arguments]]
...
- [[#web-csplayer----csplayerperfhere-cslineinvokerhere-csplayeruihere][Web csPlayer -- csPlayerPerf.here, csLineInvoker.here, csPlayerUi.here]]
  - [[#cslineinvoker][csLineInvoker]]
  - [[#csplayerperf][csPlayerPerf]]
...
Dating back to K&R and getopt and argc and argv, arg[0] was the command line and the reset were arguments.

In there, were simplistic references to options and parameters and mixed with it were
...
With BISOS-PyCS command line we introduce our own  terminology for command lines in general.
...
PyCS command-line has the follwoing general structure:
...
commandParameters are spcific to the specified commandName.

The general form of Parameters, Options and Arguments  are:


* General form of PyCS Parameters, Options and Arguments


With PyCS, we make a clear destinction between parameters and arguments.
Options are a special form of Parameters which do not take values.
...
| Short From Option    | Options don't have values     | -h           |                  |
|                      |                               | -v (verbose) |                  |
|----------------------+-------------------------------+--------------+------------------|
| Arguments            | Are ordered (poistional)      | file1 fileN  |                  |
...
|-------------------+--------------------+-----------------+------------------------------------------|
| csxu              | Command Services   | facter.cs       | The executable of CS itself              |
|                   | eXecution Unit     |                 |                                          |
|-------------------+--------------------+-----------------+------------------------------------------|
...
|-------------------+--------------------+-----------------+------------------------------------------|
| csxuParams        | csxu Prameters     | --log=30        |                                          |
|-------------------+--------------------+-----------------+------------------------------------------|
...
|-------------------+--------------------+-----------------+------------------------------------------|
| csxuModProcessors | csxu Modules       | facterModule.cs |                                          |
|                   |                    | soncli.cs       |                                          |
|-------------------+--------------------+-----------------+------------------------------------------|
...
|-------------------+--------------------+-----------------+------------------------------------------|
| csMod             | CS Module          |                 | Union of csxuModules+AdoptedModules      |
|                   |                    |                 | csMod maps to a csxuModProcessor as well |
|-------------------+--------------------+-----------------+------------------------------------------|
| csModParams       | CS Mod Parameters  |                 |  Params Specific to Module               |
|-------------------+--------------------+-----------------+------------------------------------------|
| xuSet             |                    |                 | A set of related XUs                     |
|-------------------+--------------------+-----------------+------------------------------------------|
| xuSetTree         |                    |                 | A set of related heierachical  XUs       |
|-------------------+--------------------+-----------------+------------------------------------------|
...
* Web csPlayer -- csPlayerPerf.here, csLineInvoker.here, csPlayerUi.here
